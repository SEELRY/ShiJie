...
Page({
  ...
  loaddata: function() {
    ...
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/Api/getIndexDataList', {
      ...
      for (var i = 0; i < that.data.time.length; i++) {
        if (that.data.time[i].name == 'seckill' && that.data.time[i].config.seckill_config != null && that.data.time[i].config.seckill_config.is_today == 1 && (that.data.index_data.community_config == undefined || that.data.index_data.community_config.is_open != 1)) {
          if (k == 0) {
            k++;
            that.setseckilltime()
  ...
  setseckilltime: function() {
    ...
    for (var index = 0; index < time.length; index++) {
      if (time[index].name == 'seckill' && time[index].config.seckill_config != null) {
        ...
        console.log('seckilltime2', seckilltime2);
        console.log('_Data', _Data.getHours() * 3600 + _Data.getMinutes() * 60);
  ...
  onShareAppMessage: function() {
    ...
    if (that.data.index_data.community_config && that.data.index_data.community_config.is_open == 1) {
      return {
        ...
        path: 'pages/shop/mall/mall?other_uid=' + that.data.all_address.default_colonel_uid + '&log_other_uid=' + log_other_uid,
      }
    } else {
      return {
        title: that.data.shareInfo.title,
        path: 'pages/shop/mall/mall?log_other_uid=' + log_other_uid,
      }
    }
  ...
  getCarListNum: function() {
    var that = this;
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/DuoguanShopApi/getCartList.html', {type:2},
      (data) => {
        var cart_num = 0;
        if (data) {
